...
class ExperimentRunner:
    def __init__(self, config: MountainCarConfig, allocation_strategy):
        ...
        self.envs = [
            self.factory.make_vec_env(shaping_type=c, n_envs=config.n_envs, training=True)
            for c in config.candidates
        ]
        self.models = [PPO("MlpPolicy", env, **self._ppo_kwargs()) for env in self.envs]
        self.stats = [{"mean": -200.0, "var": 25.0} for _ in config.candidates]
    ...
    def run(self, strategy_name: str):
        print(f"\n{'=' * 70}\n启动 [{strategy_name.upper()}] MountainCar 预算分配实验\n{'=' * 70}")
